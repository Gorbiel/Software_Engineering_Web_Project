...
class UserViewTests(APITestCase):
    ...
    def test_admin_can_update_rank_by_name(self):
        self.authenticate_as(self.admin_user)

        response = self.client.patch(
            f"/api/users/{self.normal_user.id}/rank/",
            {"rank": "senior"},
            format="json",
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["user_id"], self.normal_user.id)
        self.assertEqual(response.data["rank"], User.Rank.SENIOR.value)
        self.assertEqual(response.data["rank_name"], "senior")

        self.normal_user.refresh_from_db()
        self.assertEqual(self.normal_user.rank, User.Rank.SENIOR.value)

    def test_admin_can_update_rank_by_number(self):
        self.authenticate_as(self.admin_user)
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["rank_name"], "junior")

        self.normal_user.refresh_from_db()
        self.assertEqual(self.normal_user.rank, User.Rank.JUNIOR.value)

    def test_non_admin_cannot_update_rank(self):
        self.authenticate_as(self.normal_user)

        response = self.client.patch(
            f"/api/users/{self.admin_user.id}/rank/",
            {"rank": "mid"},
            format="json",
        )
