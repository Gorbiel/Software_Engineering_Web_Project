...
class LeaderboardViewSet(viewsets.ViewSet):
    ...
    def users(self, request):
        ...
        queryset = queryset.annotate(
            ...
            confirmations_received=Count(
                "achievement__achievementconfirmation", distinct=True
            ),
            confirmations_given=Count("achievementconfirmation", distinct=True),
    ...
    def my_position(self, request):
        ...
        total_score_position = (
            ...
            .filter(total_score__gt=user_data.total_score)
            .count()
            + 1
        )
        ...
        achievements_position = (
            User.users.active()
            .annotate(achievement_count=Count("achievement", distinct=True))
            .filter(achievement_count__gt=user_data.achievement_count)
            .count()
            + 1
        )
        ...
        glazes_position = (
            User.users.active()
            .annotate(glazes_received_count=Count("receiver", distinct=True))
            .filter(glazes_received_count__gt=user_data.glazes_received_count)
            .count()
            + 1
        )
