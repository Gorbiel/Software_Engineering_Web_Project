...
def get_user_stats(user: User):
    ...
    stats["latest_achievements"] = [
        a.id
        for a in Achievement.achievements.by_user(user).order_by("-creation_date")[:5]
    ]

    stats["achievement_confirmation_scores"] = [
        {"achievement_id": a.id, "confirmation_count": a.confirmation_count}
        for a in Achievement.achievements.by_user(
            user
    ...
    stats["top_achievement_confirming_users"] = [
        ...
        for c in AchievementConfirmation.confirmations.filter(
            achievement__user__exact=user
        )
    ...
    stats["achievement_reaction_counts"] = [
        ...
        for a in Achievement.achievements.with_reaction_count().filter(user__exact=user)
    ]

    stats["top_achievement_reactions"] = [
        {"reaction_id": r["reaction"], "reaction_count": r["reaction_amount"]}
        for r in AchievementReaction.objects.filter(achievement__user__exact=user)
        .values("reaction")
    ...
    stats["latest_received_glazes"] = [
        g.id for g in Glaze.glazes.received_by(user).order_by("-creation_date")[:5]
    ]

    stats["most_glazed_by"] = [
        {"user_id": g["posting_user"], "glaze_count": g["glaze_count"]}
        for g in Glaze.glazes.received_by(user)
        .values("posting_user")
    ...
    stats["top_received_glaze_reactions"] = [
        ...
        for r in GlazeReaction.objects.filter(glaze__receiving_user__exact=user)
        .values("reaction")
    ...
    stats["top_received_glaze_tags"] = [
        ...
        for r in GlazeTag.objects.filter(glaze__receiving_user__exact=user)
        .values("tag")
    ...
    stats["latest_sent_glazes"] = [
        g.id for g in Glaze.glazes.sent_by(user).order_by("-creation_date")[:5]
    ]

    stats["top_sent_glaze_reactions"] = [
        {"reaction_id": r["reaction"], "reaction_count": r["reaction_count"]}
        for r in GlazeReaction.objects.filter(glaze__posting_user__exact=user)
        .values("reaction")
        .annotate(reaction_count=Count("id"))
        .order_by("-reaction_count")[:5]
    ]
    ...
    stats["total_confirmations"] = (
        ...
        .with_weighted_confirmation_score(user)
        .aggregate(Sum("confirmation_score"))
    )["confirmation_score__sum"]
