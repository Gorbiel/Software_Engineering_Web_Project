...
import { useConfirmationRequests } from "@/context/ConfirmationRequestsContext";
import {
  clearConfirmationRequests,
  deleteConfirmationRequest,
} from "@/utils/achievements";

type ConfirmationsCardProps = {
  className?: string;
...
export function ConfirmationsCard({
  ...
  if (hideWhenEmpty && requests.length === 0) {
    return null;
  }
  ...
  return (
    <div
      className={`glaze-card flex flex-col gap-4${className ? ` ${className}` : ""}`}
    >
